...
- **#47 — Taste-model drift detection.** Weekly cron job that compares recent agent interactions (deep-read selections, digest feedback, keyword hits) against current triage thresholds and interest weights to flag stale or miscalibrated parameters. Output: a brief drift report surfaced in the digest or a separate alert. Inspired by Kjosbakken (2026) pattern of automated knowledge-base staleness checks. Prerequisite for the quarterly taste-model recalibration planned in v0.3. **M, depends on persistent interaction log (#44 rolling pool), deferred post-June 11.**
*End of roadmap.*
