...
- **#47 — Taste-model drift detection with autoresearch loop.** Weekly cron job that compares recent agent interactions (deep-read selections, digest feedback, keyword hits) against current triage thresholds and interest weights to flag stale or miscalibrated parameters. Implementation pattern: Karpathy's autoresearch loop — a `program.md` (high-level instructions) drives modifications to triage/synthesis parameters, runs a measurement cycle, and commits if the quality metric improves or `git reset`s if not. Git serves as the memory/rollback mechanism (already in place). Quality metric must be multi-dimensional from the start (papers fetched + deep-read promotion rate + digest read-through rate) to prevent reward hacking via single-metric loopholes (AlphaSignal/Sakana AI DGM warning). Output: a brief drift report surfaced in the digest or a separate alert. Inspired by Kjosbakken (2026) and Karpathy autoresearch (2026). Stepping-stones archive pattern (DGM-H) flagged as reference for v0.3 supervisor design. Prerequisite for quarterly taste-model recalibration planned in v0.3. **M, depends on persistent interaction log (#44 rolling pool), deferred post-June 11.**
*End of roadmap.*
